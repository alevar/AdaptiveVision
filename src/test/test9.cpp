...
int main( int argc, char** argv )
{
  ...
  img_scene = imread( argv[2]);
  cvtColor( sourceORIG, sourceGRAY, CV_BGR2GRAY );
  cvtColor( img_scene, img_scene, CV_BGR2GRAY );
  // threshold(img_scene, img_scene, 70, 255, THRESH_BINARY);

  imshow("thresh", img_scene);

  GaussianBlur( sourceGRAY, sourceGRAY_BLUR, Size(3,3), 3.0 );
  // GaussianBlur( img_scene, img_scene, Size(3,3), 3.0 );
  ...
  copyMakeBorder(drawing,drawing,10,10,10,10,BORDER_CONSTANT);
  // GaussianBlur( drawing, drawing, Size(3,3), 3.0 );
  ...
  OrbFeatureDetector detector(700, 1.2f, 30, 15, 0, 2, 10, 10);
  OrbFeatureDetector detector2(700, 1.2f, 30, 15, 0, 2, 10, 10);
  ...
  drawKeypoints(img_object, keypoints_object, drawing, Scalar::all(255));
  imshow("test",out);
