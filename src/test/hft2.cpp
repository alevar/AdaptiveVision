...
// g++ -std=c++11 -o hft hft.cpp `pkg-config opencv --cflags --libs`
...
Mat processImage(Mat &image, Features featuresTPL){
    ...
    int val1 = 210;
    int val2 = (int)(0.01*gray.cols*gray.rows)*100;
    int val3 = (int)(0.5*gray.cols*gray.rows)*100;
    int val4 = 100;
    int val5 = 70;

    

    MSER mser(val1/(double)100,val2/(double)100,val3/(double)100,val4/(double)100,val5/(double)100);

    namedWindow("MSER CALIBRATION", WINDOW_NORMAL);
    createTrackbar("VAL1", "MSER CALIBRATION", &val1, 100);
    createTrackbar("VAL2", "MSER CALIBRATION", &val2, 100);
    createTrackbar("VAL3", "MSER CALIBRATION", &val3, 100);
    createTrackbar("VAL4", "MSER CALIBRATION", &val4, 100);
    createTrackbar("VAL5", "MSER CALIBRATION", &val5, 100);

    while(true){

        MSER mser(val1/100,(int)(val2/(double)100),(int)(val3/(double)100),val4/(double)100,val5/(double)100);
    ...
    cvDestroyWindow("MSER CALIBRATION");
    waitKey(1);
    waitKey(1);
    waitKey(1);
    waitKey(1);
...
vector<Point> maxMser(Mat *gray)
{
    ...
    // might want to add a calibration step to constructing a template
    MSER mser(21, (int)(0.01*gray->cols*gray->rows), (int)(0.5*gray->cols*gray->rows), 1, 0.7);
...
//===================================================
    ...
    while(true){
        ...
        MSER mser(val1/(double)100,(int)(val2*gray.cols*gray.rows), (int)(val3*gray.cols*gray.rows),val4/(double)100,val5/(double)100);

        namedWindow("MSER CALIBRATION", WINDOW_NORMAL);
        createTrackbar("VAL1", "MSER CALIBRATION", &val1, 1000);
        createTrackbar("VAL2", "MSER CALIBRATION", &val2, 100);
        createTrackbar("VAL3", "MSER CALIBRATION", &val3, 1000);
        createTrackbar("VAL4", "MSER CALIBRATION", &val4, 1000);
        createTrackbar("VAL5", "MSER CALIBRATION", &val5, 1000);

        while(true){

            cap >> inputSCN;

            MSER mser(val1/100,(int)((val2/(double)1000)*gray.cols*gray.rows), (int)((val3/(double)1000)*gray.cols*gray.rows),val4/(double)100,val5/(double)100);
            // MSER mser(21, (int)(0.01*gray.cols*gray.rows), (int)(0.5*gray.cols*gray.rows), 1, 0.7);

            cout << (int)((val2/(double)1000)*gray.cols*gray.rows) << "=====" << (int)((val3/(double)1000)*gray.cols*gray.rows) << endl;
       
            cvtColor(inputSCN, gray, CV_BGRA2GRAY);
            vector<vector<Point> > msers;
            // detectRegions(gray,msers);
            mser(gray, msers);
            vector<Point> *bestMser = NULL;
            ...
            if (bestMser)
            {
                ...
                Rect bound = boundingRect(*bestMser);
                rectangle(inputSCN, bound, GREEN, 3);
                imshow("MSER CALIBRATION",inputSCN);
                waitKey(0);
            }
